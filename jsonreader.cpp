...
/*!
 ...
 \return none
*/
void wxJSONReader::StoreValue(int ch, const wxString &key, wxJSONValue &value, wxJSONValue &parent)
{
...
int wxJSONReader::SkipComment(wxInputStream &is)
{
    ...
    if (ch == '/')
    { // C++ comment, read until end-of-line
    ...
    { // it is not a comment, return the character next the first '/'
...
void wxJSONReader::StoreComment(const wxJSONValue *parent)
{
    ...
    { // comment BEFORE can only be added to the 'next' value
        if (m_next)
        {
...
*/
int wxJSONReader::ConvertCharByChar(wxString &s, const wxMemoryBuffer &utf8Buffer)
{
...
/*!
 ...
 @return TRUE if the conversion succeeds
*/
bool wxJSONReader::DoStrto_ll(const wxString &str, wxUint64 *ui64, wxChar *sign)
{
